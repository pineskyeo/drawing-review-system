...
DWG 또는 DXF 파일 입력
        │
        ▼
┌─ 도면 추출 ──────────────────────────── 토큰 사용 없음 (Python)
│  DWG → DXF 변환 (변환기 있을 때만)
│  DXF → 텍스트/원/블록 추출 → JSON
│
├─ 규칙 로드 ──────────────────────────── 토큰 사용 없음 (Python)
│  vault에서 검수완료 규칙 읽기
│
├─ 코드 검증 ──────────────────────────── 토큰 사용 없음 (Python)
│  7개 규칙 자동 체크 (PE 케이블, POLE 수, 파츠명 등)
│  → 위반 0개면 여기서 끝!
│
├─ [위반 있을 때만] 관련 규칙 검색 ────── 토큰 사용 없음 (로컬 임베딩)
│  ChromaDB에서 위반 항목과 관련된 규칙 Top-5 검색
│
├─ [위반 있을 때만] LLM 판단 ─────────── ⚡ 여기서만 토큰 사용
│  위반 항목 + 관련 규칙 5개만 프롬프트에 포함
│  → 위반 1건당 약 1,000~2,000 토큰
│
└─ 리포트 생성 ────────────────────────── 토큰 사용 없음 (Python)
   Markdown 리포트 → vault에 자동 저장
```

**핵심: 위반이 없으면 토큰 사용 0.** 위반이 있어도 해당 건수만큼만 소량 사용.
...
**Q: DWG 파일도 되나요?**
A: 네, 됩니다. DWG 변환기(LibreDWG 또는 ODA File Converter)가 설치되어 있으면 DWG를 자동으로 DXF로 변환합니다. 변환기가 없으면 AutoCAD에서 Ctrl+Shift+S → DXF로 저장해서 넣어주세요.

**Q: 토큰(비용)이 많이 드나요?**
A: 거의 안 듭니다. 전체 과정 중 **위반이 발견됐을 때 LLM 판단 단계에서만** 토큰을 사용합니다. 위반이 없으면 토큰 사용 0입니다. 위반이 있어도 관련 규칙 5개(약 1,000~2,000 토큰)만 사용하므로, 100페이지 기준서를 통째로 넣는 것 대비 97% 절감됩니다.

**Q: 인터넷 없이 되나요?**
A: 최초 설치 시에만 인터넷이 필요합니다 (패키지 다운로드). 이후 코드 검증은 오프라인으로 됩니다. LLM 판단 기능만 인터넷이 필요하고, Ollama(로컬 LLM)를 쓰면 완전 오프라인도 가능합니다.
